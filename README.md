...
- 文档分块工作台：按页码顺序分页浏览全部分块，可按状态筛选（全部 / 待审核 / 通过 / 已删除）与一键通过；确认向量化后仍可逐块编辑、删除、相邻合并与插入新块，操作即时单块向量化，无需整文档重解析
