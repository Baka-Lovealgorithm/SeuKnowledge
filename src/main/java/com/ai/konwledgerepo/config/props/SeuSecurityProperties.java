 ...
 * @param loginFailMax         连续登录失败达此数触发临时锁定（默认 5）
 * @param loginFailWindowSeconds 登录失败计数窗口（秒，默认 900=15 分钟）
 * @param loginLockSeconds     登录锁定时长（秒，默认 900=15 分钟）
 */
...
public record SeuSecurityProperties(
        ...
        @DefaultValue("5") int loginFailMax,
        @DefaultValue("900") long loginFailWindowSeconds,
        @DefaultValue("900") long loginLockSeconds) {
}
