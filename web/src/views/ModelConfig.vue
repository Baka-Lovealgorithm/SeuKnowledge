...
const USAGE = { EXTRACT: '抽取', GENERATE: '生成', RETRIEVE: '检索', VISION: '识图', RERANK: '重排', VERIFY: '校验', TITLE: '标题', ROUTER: '路由' }
const usageLabel = (u) => USAGE[u] || u

// 模型类型切换时清掉不兼容的用途绑定（如从 CHAT 切到 RERANK 时残留 GENERATE）
watch(() => form.modelType, (t) => {
  const valid = { CHAT: ['EXTRACT', 'GENERATE', 'VERIFY', 'ROUTER'], EMBEDDING: ['RETRIEVE'], VISION: ['VISION'], RERANK: ['RERANK'], TITLE: ['TITLE'] }[t] || []
