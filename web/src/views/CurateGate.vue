<template>
  <div>
    <div class="toolbar">
      ...
      <el-tag v-else type="info">未在策展流程</el-tag>
      <span class="stat">共 {{ chunks.length }} chunk ｜ SUSPECT {{ suspectCount }}</span>
      <div class="spacer" />
      <template v-if="status === 'PREVIEWING'">
...
async function load() {
  loading.value = true
  try {
    const [i, cs] = await Promise.all([curateApi.info(docId), curateApi.chunks(docId)])
    info.value = i
    chunks.value = cs
  } finally {
    loading.value = false
...
async function openMd() {
  ...
  try {
    mdText.value = await curateApi.getMd(docId)
...
async function saveMd() {
  ...
  mdSaving.value = true
  try {
    const r = await curateApi.saveMd(docId, mdText.value)
...
async function saveEdit() {
  ...
  try {
    await curateApi.editChunk(docId, editRow.value.chunkId, { content: editContent.value, title: editTitle.value })
...
async function keep(row) {
  try {
    await curateApi.keepChunk(docId, row.chunkId)
...
async function drop(row) {
  ...
  try {
    await curateApi.dropChunk(docId, row.chunkId)
