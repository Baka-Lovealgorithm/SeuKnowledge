param(
    [string]$OutFile = 'target/route-hardening-result.json',
    [string]$QuestionFile = 'test-data/route-hardening-4.json',
...
foreach ($item in $qs) {
    ...
    if ($ask.code -ne 0) {
        ...
        $firstLine = ($ask.data.answer -split "`n")[0]
        "Q$($item.n) sid=$sid done in $($sw.ElapsedMilliseconds)ms intent=$($ask.data.intent) cat=$($item.cat) chars=$($ask.data.answer.Length)"
        "  首行: $firstLine"
    }
